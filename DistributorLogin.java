...
public class DistributorLogin extends JFrame implements ActionListener {
    ...
    public DistributorLogin() {
        setTitle("Distributor Login - AgriConnect");
        setSize(400, 300);
        ...
        JLabel title = new JLabel("Distributor Login", SwingConstants.CENTER);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == loginButton) {
            ...
            int distributorId = validateLogin(email, password);
            if (distributorId != -1) {
                JOptionPane.showMessageDialog(this, "Login Successful!");
                new DistributorDashboard(distributorId).setVisible(true); // open dashboard
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Invalid Email ID or Password", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } else if (e.getSource() == backButton) {
            new LoginPage().setVisible(true); // go back to main login page
            dispose();
        }
    }
    ...
    private int validateLogin(String email, String password) {
        ...
        try (Connection con = DriverManager.getConnection(
            ...
            String query = "SELECT distributor_id FROM distributors WHERE email=? AND password=?";
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new DistributorLogin().setVisible(true));
    }
}
