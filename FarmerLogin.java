...
public class FarmerLogin extends JFrame implements ActionListener {
    ...
    public FarmerLogin() {
        setTitle("Farmer Login - AgriConnect");
        setSize(400, 300);
        ...
        JLabel title = new JLabel("Farmer Login", SwingConstants.CENTER);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == loginButton) {
            ...
            int farmerId = validateLogin(email, password);
            if (farmerId != -1) {
                JOptionPane.showMessageDialog(this, "Login Successful!");
                new FarmerDashboard(farmerId).setVisible(true); // Open dashboard for logged-in farmer
                dispose();
            } else {
    ...
    private int validateLogin(String email, String password) {
        ...
        try (Connection con = DriverManager.getConnection(
            ...
            String query = "SELECT farmer_id FROM farmers WHERE email=? AND password=?";
